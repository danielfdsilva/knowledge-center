...
export default class SelectionList {
  ...
  setList (data) {
    ...
    if (list.size !== data.length) {
      throw new Error('List is not unique. This may happen because the listKeyAccessor is not returning unique keys for the list items.');
    }
    this.__list = data;
    this.list = list;
    return this;
  ...
  setSelected (data) {
    ...
    if (selected.size !== data.length) {
      throw new Error('Selected is not unique. This may happen because the itemKeyAccessor is not returning unique keys for the selected items.');
    }
  ...
  getSelectedItems () {
    const selection = [...this.selected].map(key => {
      ...
      const selectedFromList = this.__list.find(lItem =>
        this._listKeyAccessor(lItem) === key
      );
      if (!selectedFromList) {
  ...
  preselectRange (itemKey) {
    ...
    const itemIdxOnList = listArr.findIndex(listItemkey => listItemkey === itemKey);
    if (itemIdxOnList === -1) throw new Error('New selected item is not in list');
    ...
    if (selectedArr.length) {
      ...
      lastSelectedIdxOnList = listArr.findIndex(listItemkey => listItemkey === lastSelected);
      if (lastSelectedIdxOnList === -1) throw new Error('Last selected item is not in list');
    }
  ...
  toggle (item, modifiers = {}) {
    ...
    if (!meta && !shift) {
      ...
      const allSelected = selectionRange.every(selItem => selectedArr.includes(selItem));
